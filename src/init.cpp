...
void Shutdown(void* parg)
{
    ...
    RenameThread("cultofthedeadcow-shutoff");
...
//        CTxDB().Close();
        ...
        Sleep(50);
        printf("CultoftheDeadCow exited\n\n");
        fExit = true;
#ifndef QT_GUI
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("CultoftheDeadCow version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  cultofthedeadcowd [options]                     " + "\n" +
                  "  cultofthedeadcowd [options] <command> [params]  " + _("Send command to -server or cultofthedeadcowd") + "\n" +
                  "  cultofthedeadcowd [options] help                " + _("List commands") + "\n" +
                  "  cultofthedeadcowd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "cultofthedeadcow:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("CultoftheDeadCow"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("CultoftheDeadCow"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: cultofthedeadcow.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: cultofthedeadcowd.pid)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 1984 or testnet: 0)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
 ...
 //   SoftSetArg("-addnode", "45.63.10.53");
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  CultoftheDeadCow is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("CultoftheDeadCow version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "CultoftheDeadCow server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("CultoftheDeadCow"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of CultoftheDeadCow") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart CultoftheDeadCow to complete") << "\n";
